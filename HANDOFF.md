...
- **문의 폼(2종) 백엔드 연결됨**:
  - **Contact 폼**(개발사·퍼블리셔) → **Apps Script**(`config.ts` `CONTACT_ENDPOINT`, `docs/contact-apps-script.gs`) → `퍼블리싱 문의 접수` 시트 + **게임 소개서 PDF**(선택·≤10MB)→드라이브 + biz@gahee.net 메일. 엔드포인트 직접 POST 테스트 `{ok:true}` 확인(2026-07-10). 폼 필드=이름·회사·이메일·게임명·영상·스토어·소개내용+PDF(장르/플랫폼/출시상태는 PDF에 포함돼 제거). *(Formspree(`FORM_ENDPOINT`)는 엔드포인트 없을 때만 쓰는 코드 폴백으로 남음 → 현재 미사용.)*
...
2. **⏳ Contact 폼 테스트 접수 정리** — Claude가 라이브로 보낸 테스트 2건(시트행 "[테스트]…"·"[테스트2]…", 드라이브 PDF 2개, biz 메일) 확인·삭제. + (선택) 브라우저 확장 연결 시 실제 사이트 UI 클릭 제출 테스트. (사용자) *※Formspree 활성화는 이제 불필요 — Contact가 Apps Script로 전환됨.*
